...
train_input = train[-which(names(train)=="passed")]
...
#rf = randomForest(as.factor(passed) ~ .-G3-G2-G1, data=train)
#varImpPlot(rf, sort=TRUE)
library(caret)
library(e1071)
library(DMwR)
balanced = DMwR::SMOTE(as.factor(passed) ~ .-G3-G2-G1, train, perc.over=100,perc.under=450)
...
balancedRf = randomForest(as.factor(passed) ~ .-G3-G2-G1, data=balanced,ntree=1000)
